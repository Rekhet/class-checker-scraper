...
case "$COUNT_MODE" in
  ...
  counts)
    ...
    ;;
  *)
    echo "error: COUNT_MODE must be cart or counts" >&2
    exit 2
    ;;
esac

if [ "${DRY_RUN:-}" = "1" ]; then
  echo "$ROOT/refresh.sh --year $COUNT_YEAR --counts-only --windowed ${EXTRA[*]:-} $COUNT_SEM"
  exit 0
fi

"$ROOT/refresh.sh" --year "$COUNT_YEAR" --counts-only --windowed \
  "${EXTRA[@]}" "$COUNT_SEM"
