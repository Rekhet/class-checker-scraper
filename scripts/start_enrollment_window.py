...
def parse_time(value: str) -> dt.time:
    """Parse a ten-minute-aligned local time in HH:MM form."""
    ...
    if parsed.minute % 10:
        raise ValueError(f"time must fall on a ten-minute boundary: {value!r}")
    return parsed


def build_collection_schedule(
    dates: tuple[dt.date, ...], start_time: dt.time, end_time: dt.time
) -> tuple[dt.datetime, ...]:
    """Return every inclusive ten-minute collection timestamp for each date."""
    if not dates:
        raise ValueError("at least one collection date is required")
    ...
    if span_seconds % (10 * 60):
        raise ValueError("collection time span must be divisible by ten minutes")

    schedule: list[dt.datetime] = []
    for day in dates:
        current = dt.datetime.combine(day, start_time)
        final = dt.datetime.combine(day, end_time)
        while current <= final:
